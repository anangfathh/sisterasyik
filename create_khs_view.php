                    ...
                    <div class="box">
                        <div class="box-body">
                            <div class="auth-content my-auto">
                                <form class="mt-5 pt-2" method="post" action="create_khs_process.php">
                                    <div class="mb-24">
                                        <label class="form-label mb-14">NIM</label>
                                        <select class="form-select" id="nim" name="nim">
                                            <?php
                                            require 'koneksi.php';
                                            $sql_nim = "SELECT nim FROM mahasiswa";
                                            $result_nim = $conn->query($sql_nim);
                                            if ($result_nim->rowCount() > 0) {
                                                while ($row_nim = $result_nim->fetch(PDO::FETCH_ASSOC)) {
                                                    echo "<option value='" . $row_nim["nim"] . "'>" . $row_nim["nim"] . "</option>";
                                                }
                                            } else {
                                    ...
                                    <div class="mb-24">
                                        ...
                                        <select class="form-select" id="kode_mk" name="kode_mk">
                                            <?php
                                            $sql_kode_mk = "SELECT kode_mk, nama FROM matkul";
                                            $result_kode_mk = $conn->query($sql_kode_mk);
                                            if ($result_kode_mk->rowCount() > 0) {
                                                while ($row_kode_mk = $result_kode_mk->fetch(PDO::FETCH_ASSOC)) {
                                                    echo "<option value='" . $row_kode_mk["kode_mk"] . "'>" . $row_kode_mk["nama"] . "</option>";
                                                }
                                            } else {
                                                echo "<option value=''>Tidak ada Kode MK tersedia</option>";
                                            }
                                            ?>
                                        </select>
                                    </div>
                                    <div class="mb-24">
                                        <label class="form-label mb-14">Nilai</label>
